            ...
            <CSSTransition in={inProp} timeout={200} classNames='slide' unmountOnExit={true}>
                ...
                <section id='contact' className='w-full h-full flex flex-col justify-center items-center'>
                    ...
                    <div className='h-1/3 w-full flex items-center justify-center flex-col gap-4'>
                        ...
                        <p className='text-white text-xs md:text-base'>Interested in all kinds of opportunities and willing to learn and improve</p>
                    </div>

                    <form onSubmit={(e) => handleSubmit(e)} className='flex flex-col h-2/3 text-white text-xl gap-5 w-full lg:w-2/3'>

                        <div className='flex w-2/3 items-start mx-auto'>
                            <input onChange={(e) => setEmail(e.target.value)} value={email} className='p-2 w-full md:w-3/4 bg-secondary text-base text-white rounded outline-none border-2 border-transparent transition-all ease-in-out 350 hover:border-tertiary focus:border-tertiary' type="email" name='email' id='email' required placeholder='john@doe.com' />
                        </div>

                        <div className='flex w-2/3 items-start mx-auto gap-2'>
                            <input onChange={(e) => setFirstName(e.target.value)} value={firstName} className='p-2 bg-secondary w-2/3 md:w-1/3 text-base text-white rounded outline-none border-2 border-transparent transition-all ease-in-out 350 hover:border-tertiary focus:border-tertiary' type="text" name='firstName' id='first-name' required placeholder='John' />
                            <input onChange={(e) => setLastName(e.target.value)} value={lastName} className='p-2 bg-secondary w-2/3 md:w-1/3 text-base text-white rounded outline-none border-2 border-transparent transition-all ease-in-out 350 hover:border-tertiary focus:border-tertiary' type="text" name='lastName' id='last-name' required placeholder='Doe' />
                        </div>
